...
# ERSIN Global model service instance
...
def train_model_route (model_type ):
    ...
    if model_type not in ['content','age']:
        return jsonify ({"error":"Invalid model type"}),400 
    try :
    # ERSIN Eğitim parametreleri request body'den alınabilir (örn: epoch sayısı vb.)
    # ERSIN Şimdilik varsayılan parametrelerle çalışacak
    # ERSIN training_params = request.json if request.is_json else {}

        current_app .logger .info (f"{model_type } modeli için eğitim süreci başlatılıyor (manuel tetikleme).")
        success ,message_or_metrics =model_service .train_with_feedback (model_type )# ERSIN training_params buraya eklenebilir
...
def load_model_version_route (model_type ,version_id ):
    ...
    try :
        ...
        if success :
            ...
            return jsonify ({"error":message }),404 # ERSIN veya 500 duruma göre
    except Exception as e :
...
def cleanup_system ():
    ...
    try :
        ...
        # ERSIN Kapsamlı temizlik gerçekleştir
...
def cleanup_feedback ():
    ...
    try :
        data =request .get_json ()or {}
        model_type =data .get ('model_type','age')# ERSIN 'age' veya 'content'
...
def get_cleanup_status ():
    ...
    # ERSIN Ensemble feedback istatistikleri
        ...
        # ERSIN Ensemble klasör boyutları
        ...
        def get_dir_size (path ):
            ...
            return total_size /(1024 *1024 )# ERSIN MB
...
def delete_latest_model_version_route (model_type ):
    ...
    if model_type not in ['content','age']:
        return jsonify ({"error":"Invalid model type"}),400 
    try :
    # ERSIN ModelService'de mevcut metod adı farklı
...
def switch_model_version_route (model_type ):
    ...
    try :
        ...
        # ERSIN ModelService'de yeni eklenen metod
