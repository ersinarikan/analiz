...
# ERSIN TensorFlow uyarılarını bastır - lazy import (circular import'u önlemek için)
import warnings 
warnings .filterwarnings ('ignore',category =DeprecationWarning )
# ERSIN TensorFlow import'u lazy yap - sadece gerektiğinde yükle
def _get_tensorflow ():
    """Lazy TensorFlow import to avoid circular import issues"""
    try :
        import tensorflow as tf 
        # ERSIN Güncel TensorFlow 2.x logging API'si kullan
        if hasattr (tf ,'get_logger'):
            tf .get_logger ().setLevel ('ERROR')
        return tf 
    except (ImportError ,AttributeError )as e :
        logger .warning (f"TensorFlow yüklenemedi: {e }")
        return None 
...
def test_content_detection ():
    ...
    try :
    # ERSIN Test nesnesi oluştur
        detection =ContentDetection (
        analysis_id =1 ,
        frame_path ="test_frame.jpg"
        )

        # ERSIN Rastgele skor değerleri ata
        ...
        # ERSIN NumPy değerler içeren nesneler listesi oluştur
        detected_objects =[
        {
        'label':'person',
...
def system_info ():
    ...
    try :
        ...
        # ERSIN Python ve kütüphane bilgileri
        python_info ={
        'version':sys .version ,
        'platform':platform .platform (),
        'numpy_version':np .__version__ ,
        'cv2_version':cv2 .__version__ ,
        'tensorflow_version':tf .__version__ if tf else "Yüklenemedi"
        }

        # ERSIN İşletim sistemi ve donanım bilgileri
        system ={
        'os':platform .system (),
        'release':platform .release (),
        'cpu_count':os .cpu_count (),
        'memory_total':psutil .virtual_memory ().total /(1024 **3 ),# ERSIN GB
        'memory_available':psutil .virtual_memory ().available /(1024 **3 )# ERSIN GB
        }

        # ERSIN GPU bilgisi (NVIDIA için)
        gpu_info ={}
        try :
...
def uploaded_files_count ():
    ...
    try :
    # ERSIN Son 24 saat içinde yüklenen dosyaları kontrol et
        cutoff_time =datetime .utcnow ()-timedelta (hours =24 )

        # ERSIN Son 24 saat içinde yüklenen dosyaları al
        recent_files =File .query .filter (File .created_at >=cutoff_time ).all ()
        recent_file_ids =[f .id for f in recent_files ]

        # ERSIN Bu dosyalardan tamamlanmış analizlere sahip olanları bul
        if recent_file_ids :
            from app .utils .sqlalchemy_helpers import column_in 
            completed_analyses =Analysis .query .filter (
            column_in (Analysis .file_id ,recent_file_ids ),
            Analysis .status =='completed'
            ).all ()
            completed_file_ids =set (analysis .file_id for analysis in completed_analyses )

            # ERSIN Aktif analizlere sahip dosyaları bul (processing, queued)
            from sqlalchemy import and_ 
            from app .utils .sqlalchemy_helpers import column_in 
            active_analyses =Analysis .query .filter (
            and_ (
            column_in (Analysis .file_id ,recent_file_ids ),
            column_in (Analysis .status ,['processing','queued','pending'])
            )
            ).all ()
            active_file_ids =set (analysis .file_id for analysis in active_analyses )

            # ERSIN Yüklü ama henüz analizi tamamlanmamış veya aktif analizi olan dosya sayısı
