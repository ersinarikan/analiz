...
def reset_model_by_type (model_type ):
    ...
    try :
        ...
        if success :
            ...
            # ERSIN Yaş modeli sıfırlandığında sistem yeniden başlatılmalı
            if model_type =='age':
                logger .info (f"{model_type } modeli sıfırlandı")

                # ERSIN Model state dosyasını güncelle
                from app .utils .model_state import update_model_state_file 
                update_model_state_file (model_type ,0 )# ERSIN 0 = base model
...
def train_model ():
    ...
    try :
        ...
        model_type =data ['model_type']
        # ERSIN epochs, batch_size, learning_rate şu an kullanılmıyor ama gelecekte kullanılabilir
        _ =data .get ('epochs',10 )
        _ =data .get ('batch_size',16 )
        _ =data .get ('learning_rate',0.001 )

        if model_type not in ['content','age']:
            return jsonify ({'error':'Geçersiz model tipi. Desteklenen tipler: content, age'}),400 

            # ERSIN Eğitim verisini hazırla
        service =ModelService ()
        ...
        if not training_data :
            ...
            # ERSIN Benzersiz model ID oluştur (şu an kullanılmıyor)
        _ =str (uuid .uuid4 ())

        # ERSIN WebSocket sistemi üzerinden real-time training yapılır
        # ERSIN training_data is a list, not a dict
        categories =[]
        if isinstance (training_data ,list )and len (training_data )>0 :
        # ERSIN Extract categories from first item if it's a dict with 'labels'
            first_item =training_data [0 ]
            if isinstance (first_item ,dict )and 'labels'in first_item :
                categories =list (first_item ['labels'].keys ())
...
def training_status (training_id ):
    ...
    """
    try :
    # ERSIN Simülasyon - Gerçek bir uygulamada veritabanından veya Redis'ten durumu alır
    # ERSIN Gerçek durum kontrolü kodu burada olacak

    # ERSIN Örnek için sabit bir ilerleme değeri (şu an kullanılmıyor)
        _ =75 
...
def get_model_versions (model_type ):
    ...
    # ERSIN service.get_model_versions zaten bir dictionary döndürüyor
...
def activate_model_version (version_id ):
    ...
    # ERSIN Frontend'in beklediği formata uygun yanıt döndür
    if 'success'in result :
        if result ['success']:
        # ERSIN Model başarıyla aktifleştirildi
            logger .info (f"Model versiyonu {version_id } aktifleştirildi")

            # ERSIN model_state.py güncellendiği için Flask otomatik restart yapacak
            response =jsonify ({
            **result ,
    ...
    # ERSIN Eğer result dictionary'de success key'i yoksa ekle
...
def train_with_feedback ():
    ...
    # ERSIN Arka planda eğitim başlat
    # ERSIN Not: Gerçek uygulamada bu işlem Celery ile asenkron çalıştırılmalı
    service =ModelService ()
...
def get_age_training_data_stats ():
    ...
    try :
        ...
        # ERSIN Veriyi hazırla ama eğitme
...
def train_model_web ():
    ...
    try :
        ...
        # ERSIN Content model training - Sadece CLIP ensemble sistemi kullan
            from app .services .ensemble_clip_service import EnsembleClipService 

            # ERSIN WebSocket session ID oluştur ve training başlama bildirimi
            content_session_id =str (uuid .uuid4 ())

            try :
                from app .routes .websocket_routes import emit_training_started 
                emit_training_started (content_session_id ,'content',100 )# ERSIN Yaklaşık sample sayısı
            except Exception as ws_err :
                logger .warning (f"WebSocket content training started event hatası: {str (ws_err )}")

                # ERSIN Sadece CLIP ensemble servisini kullan (Age sistem karışmasın)
            clip_ensemble =EnsembleClipService ()
            clip_result =clip_ensemble .load_content_corrections ()

            # ERSIN Parse  result (now returns dict ile count ve IDs)
            corrections_count_raw =clip_result ['corrections_count']
            used_feedback_ids_raw =clip_result ['used_feedback_ids']
            # ERSIN Type checker için corrections_count'u int olarak cast et
            corrections_count =int (corrections_count_raw )if isinstance (corrections_count_raw ,(int ,float ))else 0 
            # ERSIN Type checker için used_feedback_ids'ı list[int] olarak cast et
            used_feedback_ids =cast (list [int ],used_feedback_ids_raw )if isinstance (used_feedback_ids_raw ,list )else []

            logger .info (f"📊 CLIP corrections loaded: {corrections_count }")
            logger .info (f"📊 Used feedback IDs for cleanup: {len (used_feedback_ids )}")

            # ERSIN CLIP ensemble versiyonu oluştur (eğer düzeltme varsa)
            clip_version =None 
            ...
            if corrections_count >0 :
                ...
                # ERSIN ✅ CLEANUP: Kullanılan eğitim verilerini temizle
                ...
                # ERSIN WebSocket ile content training tamamlanma bildirimi
            try :
        ...
        # ERSIN Age model training - Artık destekleniyor!
            from app .services .age_training_service import AgeTrainingService 

            trainer =AgeTrainingService ()

            # ERSIN Veriyi hazırla (API'den gelen min_samples parametresini kullan)
            ...
            if training_data is None :
                ...
                # ERSIN WebSocket ile progress tracking için session ID oluştur
            training_session_id =str (uuid .uuid4 ())

            # ERSIN WebSocket ile training başlatma bildirimi
            try :
                from app .routes .websocket_routes import emit_training_started 
                ...
                # ERSIN Background task olarak eğitimi başlat
            from threading import Thread 
            training_thread =Thread (
            target =_run_age_training ,
            args =(trainer ,training_data ,params ,training_session_id ,getattr (current_app ,'_get_current_object',lambda :current_app )())
            )
...
def _run_content_training (trainer ,training_data ,params ,session_id ,app ):
    ...
    """
    # ERSIN Flask app context'ini arka plan thread'e taşı
    with app .app_context ():
        try :
        # ERSIN Session ID'yi params'a ekle
            params ['session_id']=session_id 

            # ERSIN Eğitimi başlat
            ...
            # ERSIN SQLAlchemy objesinin attribute'larını serialize et
            version_name =model_version .version_name 

            # ERSIN WebSocket ile training tamamlanma bildirimi
            try :
                from app .routes .websocket_routes import emit_training_completed 
        ...
        # ERSIN WebSocket ile training error bildirimi
            try :
...
def _run_age_training (trainer ,training_data ,params ,session_id ,app ):
    ...
    with app .app_context ():
        try :
            ...
            # ERSIN Eğitim parametrelerini hazırla
            training_params ={
            ...
            # ERSIN İlerleme callback fonksiyonu
            def progress_callback (epoch ,total_epochs ,metrics =None ):
                ...
                # ERSIN WebSocket ile progress bildirimi
                try :
                    ...
                    # ERSIN Parametrelere callback ekle
            ...
            # ERSIN 🎯 YENİ MODELİ OTOMATİK AKTİVE ET
            ...
            if activation_success :
                ...
                # ERSIN Eğitimde kullanılan verileri temizle
        ...
        # ERSIN WebSocket ile training error bildirimi
            ...
            # ERSIN Re-raise  exception için logging
            raise 
...
def _estimate_training_duration (samples ,epochs ):
    ...
    """
    # ERSIN Basit tahmin - gerçek değerler deneyime göre ayarlanmalı
    seconds_per_sample_per_epoch =0.1 # ERSIN Saniye
...
def get_training_stats (model_type ):
    ...
    try :
        if model_type =='content':
            ...
            'conflicts_detected':0 ,# ERSIN Ensemble sistemde çelişki yok
            'conflicts':[],
        ...
        # ERSIN Age model stats - AgeTrainingService kullan
            from app .services .age_training_service import AgeTrainingService 
            from app .models .feedback import Feedback 

            # ERSIN Raw feedback sayılarını al (conflict resolution öncesi)
            ...
            # ERSIN manual_count ve pseudo_count şu an kullanılmıyor
            _ =training_data ['sources'].count ('manual')
            _ =training_data ['sources'].count ('pseudo')
            ages =training_data ['ages']

            # ERSIN Yaş dağılımını hesapla
            age_distribution ={}
            for age in ages :
            ...
            'success':True ,
            'stats':{
            'total_feedbacks':total_raw_feedbacks ,# ERSIN Raw toplam (manuel + pseudo)
            'total_samples':len (training_data ['embeddings']),# ERSIN Conflict resolution sonrası
            'manual_samples':raw_manual_feedbacks ,# ERSIN Raw manuel sayısı
            'pseudo_samples':raw_pseudo_feedbacks ,# ERSIN Raw pseudo sayısı
            'age_range':{
            'min':float (ages .min ()),
            'max':float (ages .max ())
            },
...
def analyze_conflicts (model_type ):
    ...
    try :
        if model_type =='content':
        # ERSIN Ensemble sistemde çelişki analizi gerekli değil
            return jsonify ({
            'success':True ,
            'conflicts':[],
            'total_conflicts':0 ,
        ...
        # ERSIN Yaş modeli için çelişki analizi gerekli değil
            return jsonify ({
            'success':True ,
            'conflicts':[],
            'total_conflicts':0 ,
...
def test_websocket ():
    ...
    try :
        ...
        app =getattr (current_app ,'_get_current_object',lambda :current_app )()

        # ERSIN Test event'i gönder - background task ile
        ...
        if socketio :
        # ERSIN Use getattr to safely access start_background_task method
            start_task_method =getattr (socketio ,'start_background_task',None )
            if start_task_method is not None and callable (start_task_method ):
                start_task_method (emit_test_event ,app ,test_session_id )
    ...
    try :
        ...
        def emit_all_test_events (app_instance ,session_id ):
            ...
            # ERSIN 1. Basit test event
                ...
                # ERSIN 4. Test training_completed
                ...
                # ERSIN 5. Test generic event
                socketio =get_socketio ()
                if socketio :
                # ERSIN Use getattr to safely access emit method
                    emit_method =getattr (socketio ,'emit',None )
                    if emit_method is not None and callable (emit_method ):
                        emit_method ('test_event',{
        ...
        # ERSIN Use getattr to safely access start_background_task method
            start_task_method =getattr (socketio ,'start_background_task',None )
            if start_task_method is not None and callable (start_task_method ):
                start_task_method (emit_all_test_events ,app ,test_session_id )
