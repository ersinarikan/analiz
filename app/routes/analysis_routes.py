...
# ERSIN desc line 533'te fonksiyon içinde import ediliyor
from app import db 
...
def start_analysis ():
    ...
    # ERSIN JSON input validation
        ...
        try :
        # ERSIN request.json invalid olduğunda BadRequest fırlatabilir
        ...
        websocket_session_id =request .headers .get ('X-WebSocket-Session-ID')# ERSIN Header'dan WebSocket session ID'yi al
...
def get_results (analysis_id ):
    ...
    # ERSIN ID validation (negatif/sıfır ID geçersiz; UUID/string kabul)
        ...
        if results is None or 'error'in results :
            return jsonify (results or {'error':'Analiz bulunamadı'}),404 
...
def submit_feedback (analysis_id ):
    ...
    """
    try :
    # ERSIN ID validation (numeric negatives veya zero dır invalid; UUID/string ids allowed)
        if isinstance (analysis_id ,str )and analysis_id .isdigit ()and int (analysis_id )<=0 :
            return jsonify ({'error':'Geçersiz analiz ID'}),400 
...
def get_feedback (analysis_id ):
    ...
    """
    # ERSIN Analizin varlığını kontrol et (get_or_404 exception fırlatır, değişkene atama gereksiz)
...
def get_detailed_results (analysis_id ):
    ...
    try :
        ...
        # ERSIN Frontend fetchWithRetry non-2xx gördüğünde tekrar tekrar deniyor ve console spam oluyor.
        # ERSIN Bu yüzden failed/cancelled/pending için 200 dönüp status+error payload veriyoruz.
            return jsonify ({
            'analysis_id':analysis .id ,
            'status':analysis .status ,
        ...
        # ERSIN Dosya adını düzgün biçimde çıkar - frame_XXX.jpg formatındaki dosya adını al
            ...
            if '/storage/'in processed_image_path :
                ...
                # ERSIN Resim analizi için uploads klasörü kontrolü
                if path_after_storage .startswith ('uploads/'):
                # ERSIN Resim dosyası için uploads klasöründen serve et
                    processed_image_path =path_after_storage # ERSIN uploads/dosya.jpg
                else :
                # ERSIN Video kareleri için processed klasöründen serve et
                    processed_image_path =path_after_storage # ERSIN processed/frames_xxx/...
            else :
        ...
        'file_filename':analysis .file .filename if analysis .file else None ,# ERSIN Orijinal video için
        'overall_scores':{
        ...
        # ERSIN Type checker için highest_risk'ı güvenli şekilde kullan
        highest_risk =results .get ('highest_risk')
        if isinstance (highest_risk ,dict ):
            logger .info (f"[BACKEND][highest_risk] frame: {highest_risk .get ('frame')}")
            logger .info (f"[BACKEND][highest_risk] processed_image_path: {highest_risk .get ('processed_image_path')}")
...
def get_recent_analyses ():
    ...
    # ERSIN Son 10 completed analizi al (en yeni üstte)
        ...
        # ERSIN Analysis.end_time is a SQLAlchemy Column, cast for type checker
        end_time_col =cast (ColumnElement [Any ],Analysis .end_time )
        recent_analyses =Analysis .query .filter_by (
        status ='completed'
        ).order_by (desc (end_time_col )).limit (10 ).all ()

        result =[]
        for analysis in recent_analyses :
        # ERSIN completed_at için end_time ideal, ama eski kayıtlar null olabilir.
