...
def trigger_memory_cleanup ():
    ...
    try :
        ...
        # ERSIN Get memory stats after cleanup
        memory_stats =get_memory_usage ()
...
def clear_model_cache_endpoint ():
    """Clear model cache"""
    try :
        model_type =(request .json or {}).get ('model_type')if request .is_json else None 

        clear_model_cache (model_type )

        # ERSIN Get updated stats
        cache_stats =get_cache_stats ()
...
def get_performance_overview ():
    ...
    try :
    # ERSIN Collect all performance data
        memory_stats =get_memory_usage ()
        gpu_info =GPUMemoryManager .get_gpu_memory_info ()
        cache_stats =get_cache_stats ()
        db_stats =get_database_stats ()
...
def optimize_system ():
    ...
    try :
        ...
        try :
            ...
            # ERSIN Step 3: Clear query cache
        try :
            clear_query_cache ()
            ...
            # ERSIN Step 4: Clear model cache if memory usage dir high
        memory_stats =get_memory_usage ()
