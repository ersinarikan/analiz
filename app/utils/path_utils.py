...
# ERSIN Proje kök dizinini belirle (ör: WSANALIZ klasörü) - workspace root olmalı
BASE_DIR =os .path .abspath (os .path .dirname (os .path .dirname (os .path .dirname (__file__ ))))# ERSIN app/utils/ klasöründen 2 seviye yukarı çık, workspace root'a git
...
def to_rel_path (abs_path :str )->str :
    ...
    try :
        ...
        # ERSIN "../" ile başlamasını önle - eğer workspace dışındaysa absolute path döndür
        if rel_path .startswith ('..'):
            return abs_path .replace ("\\","/")
        return rel_path .replace ("\\","/")
    except ValueError :
    # ERSIN farklı drive'larda olabilir (Windows)
        return abs_path .replace ("\\","/")
