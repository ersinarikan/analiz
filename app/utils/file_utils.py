...
def is_allowed_file (filename :str ,allowed_extensions :set [str ]|None =None )->bool :
    """
    Dosya adının izin verilen uzantıya sahip olup olmadığını kontrol eder.
    Args:
        filename (str): Dosya adı.
        allowed_extensions (set[str] | None): İzin verilen uzantılar set'i. None ise varsayılan kullanılır.
    Returns:
        bool: İzin veriliyorsa True, aksi halde False.
    """
    if allowed_extensions is None :
    # ERSIN Varsayılan izin verilen uzantılar
...
def generate_unique_filename (original_filename ):
    ...
    # ERSIN Güvenli bir dosya adı oluştur
    filename =secure_filename (original_filename )

    # ERSIN Uzantıyı al
    ext =get_file_extension (filename )

    # ERSIN Benzersiz UUID oluştur
    unique_id =str (uuid .uuid4 ())

    # ERSIN Benzersiz dosya adını oluştur
...
def get_file_mimetype (file_path ):
    ...
    try :
    # ERSIN mimetypes kütüphanesini kullan
        mime_type ,_ =mimetypes .guess_type (file_path )
        if not mime_type :
        # ERSIN Fallback to common types based on extension
...
def move_file (source_path :str ,destination_path :str )->bool :
    ...
    try :
    # ERSIN Hedef dizinin var olduğundan emin ol
...
def get_file_list (directory_path :str ,filter_func =None ):
    ...
    try :
        ...
        # ERSIN Dizindeki dosyaları listele
        for filename in os .listdir (directory_path ):
            file_path =os .path .join (directory_path ,filename )

            # ERSIN Sadece dosyaları kontrol et (dizinleri atla)
            if os .path .isfile (file_path ):
            # ERSIN Eğer bir filtre fonksiyonu belirtilmişse uygula
...
def format_file_size (size_bytes :int )->str :
    ...
    # ERSIN İkinin katları için bölme katsayısı (1024)
    factor =1024 

    # ERSIN Birimleri tanımla
    size_units =["B","KB","MB","GB","TB","PB"]

    # ERSIN Boyut 1 bayttan küçükse, 0 bayt döndür
    if size_bytes <1 :
        return "0 B"

        # ERSIN Uygun birimi bul
        # ERSIN size_bytes will become float during division, use a separate variable
    size_bytes_float :float =float (size_bytes )
    i =0 
    while size_bytes_float >=factor and i <len (size_units )-1 :
        size_bytes_float /=factor 
        i +=1 

        # ERSIN Sonucu formatla (1 decimal precision)
    return f"{size_bytes_float :.1f} {size_units [i ]}"
