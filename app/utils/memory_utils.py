...
def get_memory_usage ()->Dict [str ,Any ]:
    ...
    # ERSIN System memory
        ...
        stats :Dict [str ,Any ]={
        'system':{
        'total':memory .total /(1024 **3 ),# ERSIN GB
        'available':memory .available /(1024 **3 ),# ERSIN GB
        'used':memory .used /(1024 **3 ),# ERSIN GB
        'percentage':memory .percent 
        },
        'python_process':{
        'rss':psutil .Process ().memory_info ().rss /(1024 **2 ),# ERSIN MB
        'vms':psutil .Process ().memory_info ().vms /(1024 **2 ),# ERSIN MB
        }
        }

        # ERSIN GPU memory (if available)
        if torch .cuda .is_available ():
            gpu_stats :dict [str ,dict [str ,float ]]={}
            for i in range (torch .cuda .device_count ()):
                gpu_memory =torch .cuda .memory_stats (device =i )
                # ERSIN GPU memory stats are dict[str, int], convert to float for MB
                gpu_stats [f'gpu_{i }']={
                'allocated':float (gpu_memory .get ('allocated_bytes.all.current',0 ))/(1024 **2 ),# ERSIN MB
                'reserved':float (gpu_memory .get ('reserved_bytes.all.current',0 ))/(1024 **2 ),# ERSIN MB
                'max_allocated':float (gpu_memory .get ('allocated_bytes.all.peak',0 ))/(1024 **2 ),# ERSIN MB
                }
            stats ['gpu']=gpu_stats 
        else :
        # ERSIN GPU not available, set status string
            gpu_not_available :Dict [str ,Any ]={'status':'not_available'}
            stats ['gpu']=gpu_not_available 
...
def cleanup_memory (force =False ):
    ...
    # ERSIN Minimum 30 saniye ara ile cleanup
    ...
    try :
        ...
        # ERSIN Python garbage collection
        collected =gc .collect ()
        logger .debug (f"Garbage collection: {collected } objects collected")

        # ERSIN GPU memory cleanup (if available)
        if torch .cuda .is_available ():
            torch .cuda .empty_cache ()
            logger .debug ("GPU cache temizlendi")

            # ERSIN Memory kullanımı kritik seviyede yüksekse model cache'i temizle
        ...
        # ERSIN Sadece GPU cache temizle, model instance'ları korur
            logger .warning ("High memory usage detected, clearing GPU cache only")
            if torch .cuda .is_available ():
                torch .cuda .empty_cache ()

                # ERSIN Clear database query cache only on critical memory
...
def memory_monitor (operation_name :str ,cleanup_threshold_mb :int =1000 ):
    ...
    try :
        ...
        # ERSIN Calculate memory difference
        ...
        # ERSIN Memory kullanımı yüksekse otomatik cleanup yap
        ...
        def monitor_loop ():
            while self .monitoring_enabled :
                try :
                    ...
                    time .sleep (60 )# ERSIN Wait before retry
...
class GPUMemoryManager :
    ...
    def optimize_gpu_memory ():
        ...
        try :
        # ERSIN Set memory fraction
            torch .cuda .set_per_process_memory_fraction (0.8 )

            # ERSIN etkinleştir memory pool
    ...
    def get_gpu_memory_info ()->Dict [str ,Any ]:
        ...
        for i in range (torch .cuda .device_count ()):
            ...
            allocated =torch .cuda .memory_allocated (device )/(1024 **2 )# ERSIN MB
            reserved =torch .cuda .memory_reserved (device )/(1024 **2 )# ERSIN MB
        ...
        # ERSIN Global memory manager instance
...
def initialize_memory_management ()->None :
    ...
    """
    try :
    # ERSIN Optimize GPU memory
        GPUMemoryManager .optimize_gpu_memory ()

        # ERSIN Start memory monitoring
...
def get_memory_recommendations ()->Dict [str ,Any ]:
    ...
    try :
        ...
        # ERSIN System memory recommendations
        ...
        if system_usage >90 :
            ...
            # ERSIN GPU memory recommendations
