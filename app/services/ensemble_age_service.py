...
class EnsembleAgeService :
    ...
    def __init__ (self ):
        ...
        self .feedback_corrections ={}# ERSIN person_id -> age_correction
        self .embedding_corrections ={}# ERSIN embedding_hash -> age_correction
    ...
    def load_feedback_corrections (self )->int :
        """
        ...
        try :
        # ERSIN Yaş geri bildirimi olan kayıtları al (sadece embedding olanları)
            ...
            for feedback in feedbacks :
                ...
                # ERSIN Embedding'i parse et
                    ...
                    if embedding_str :
                        ...
                        # ERSIN Sadece embedding düzeltmesi (person_id artık kullanılmıyor)
                        ...
                        'embedding':embedding ,
                        'confidence':0.95 ,
                        'person_id':feedback .person_id ,# ERSIN Sadece bilgi amaçlı (lookup'ta kullanılmıyor)
                    ...
                    # ERSIN Person ID tablosunu temizle (artık kullanılmıyor)
    ...
    def _hash_embedding (self ,embedding :np .ndarray [Any ,Any ])->int :
        ...
        # ERSIN Simple hash based on first few dimensions
        return hash (tuple (embedding [:10 ].round (3 )))

    def predict_age_ensemble (self ,base_age :float ,face_embedding :np .ndarray [Any ,Any ]|None =None )->tuple [float ,float ,dict [str ,Any ]]:
        """
        ...
        # ERSIN Embedding yoksa base model kullan
        ...
        if embedding_hash in self .embedding_corrections :
            ...
            # ERSIN 2. Similarity-based correction
        best_similarity =-1 
        ...
        for _emb_hash ,correction in self .embedding_corrections .items ():
            ...
            if similarity >best_similarity :
                ...
                # ERSIN Yüksek benzerlik varsa düzeltmeyi uygula (threshold lowered to 0.90 için better matching)
        if best_similarity >0.90 and best_correction is not None :# ERSIN %90 benzerlik eşiği (daha esnek)
            corrected_age =best_correction ['corrected_age']
            confidence =best_correction ['confidence']*best_similarity # ERSIN Benzerlik oranında güven

            logger .info (f"✅ Similarity match ({best_similarity :.3f}) -> {corrected_age } years")

            # ERSIN Kullanım sayısını artır
            if best_correction is not None and 'feedback_id'in best_correction :
                self ._increment_usage_count (best_correction ['feedback_id'])

            return corrected_age ,confidence ,{
            'method':'similarity_match',
            'similarity':best_similarity ,
            'person_id':best_correction .get ('person_id')if best_correction else None ,
            'source':best_correction .get ('source','unknown')if best_correction else 'unknown',
            'feedback_id':best_correction .get ('feedback_id')if best_correction else None 
            }
    ...
    def get_statistics (self )->dict [str ,Any ]:
        ...
        'total_people_corrections':len (self .embedding_corrections ),# ERSIN Same as embedding corrections now
    ...
    def test_ensemble_predictions (self ,test_cases =None ):
        ...
        for person_id ,correction in list (self .feedback_corrections .items ())[:5 ]:
            ...
            # ERSIN Simulate base model prediction (add some noise)
            simulated_base_pred =corrected_age +np .random .normal (0 ,2 )# ERSIN +/- 2 years noise
    ...
    def save_ensemble_corrections_as_version (self )->ModelVersion :
        ...
        # ERSIN Versiyon numarasını belirle
            ...
            # ERSIN Ensemble verilerini .pth formatında kaydet (sadece embedding-based)
            ensemble_data ={
            'model_type':'ensemble_age',
            'embedding_corrections':self .embedding_corrections ,# ERSIN Sadece embedding corrections
            'version':new_version_num ,
            'version_name':version_name ,
            'created_at':datetime .now ().isoformat (),
            'total_corrections':len (self .embedding_corrections ),# ERSIN Person ID artık yok
            'correction_method':'embedding_based_only'
            }
    ...
    def _get_correction_sources_stats (self )->dict [str ,Any ]:
        """Get statistics about correction sources (embedding-based only)"""
        sources ={}

        # ERSIN Sadece embedding corrections'dan kaynak istatistiklerini topla
    ...
    def _get_used_feedback_ids (self )->list [int ]:
        """Get list of feedback IDs used in ensemble corrections (embedding-based only)"""
        feedback_ids =[]

        # ERSIN Sadece embedding corrections'dan feedback ID'leri topla
        for correction in self .embedding_corrections .values ():
            if 'feedback_id'in correction :
                feedback_ids .append (correction ['feedback_id'])

        return list (set (feedback_ids ))# ERSIN Duplicate'leri çıkar
