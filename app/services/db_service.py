...
# ERSIN Query cache için performance optimization
_query_cache ={}
_cache_lock =threading .Lock ()
_cache_ttl =300 # ERSIN 5 minutes
...
def query_all_db (model ,**kwargs ):
    ...
    try :
        ...
        # ERSIN Performance-optimized query functions
...
def get_analysis_with_relations (analysis_id :str ,include_content_detections =True ,include_age_estimations =True ):
    ...
    try :
        ...
        # ERSIN Eager loading ile joinedload için better performance
        # ERSIN SQLAlchemy relationships work correctly, type checker doesn't recognize them
        # ERSIN Use relationship attributes directly - SQLAlchemy accepts them at runtime
        if include_content_detections :
            content_detections_attr =getattr (Analysis ,'content_detections',None )
            if content_detections_attr is not None :
                query =query .options (joinedload (content_detections_attr ))
            else :
                # ERSIN Type checker için string literal'ı cast et
                query =query .options (joinedload (cast (Any ,'content_detections')))

        if include_age_estimations :
            age_estimations_attr =getattr (Analysis ,'age_estimations',None )
            if age_estimations_attr is not None :
                query =query .options (joinedload (age_estimations_attr ))
            else :
                # ERSIN Type checker için string literal'ı cast et
                query =query .options (joinedload (cast (Any ,'age_estimations')))

                # ERSIN File relationship
        file_attr =getattr (Analysis ,'file',None )
        if file_attr is not None :
            query =query .options (joinedload (file_attr ))
        else :
            # ERSIN Type checker için string literal'ı cast et
            query =query .options (joinedload (cast (Any ,'file')))
...
def get_analyses_by_file_optimized (file_id :str ,limit :int =10 ):
    ...
    try :
        ...
        analyses =db .session .query (Analysis ).filter (Analysis .file_id ==file_id ).options (
        # ERSIN Type checker için RelationshipProperty'leri cast et
        selectinload (cast (Any ,Analysis .content_detections )),
        selectinload (cast (Any ,Analysis .age_estimations ))
        ).order_by (desc (created_at_col )).limit (limit ).all ()
...
def get_paginated_analyses (page =1 ,per_page =20 ,status_filter =None ,file_type_filter =None ):
    ...
    try :
        ...
        content_detections_attr =getattr (Analysis ,'content_detections',None )
        age_estimations_attr =getattr (Analysis ,'age_estimations',None )
        # ERSIN Type checker için cast import et
        from typing import cast 
        query =db .session .query (Analysis ).join (File ).options (
        # ERSIN Type checker için string literal'ları ve attribute'ları cast et
        joinedload (cast (Any ,file_attr ))if file_attr is not None else joinedload (cast (Any ,'file')),
        selectinload (cast (Any ,content_detections_attr ))if content_detections_attr is not None else selectinload (cast (Any ,'content_detections')),
        selectinload (cast (Any ,age_estimations_attr ))if age_estimations_attr is not None else selectinload (cast (Any ,'age_estimations'))
        )
        ...
        if file_type_filter :
            ...
            # ERSIN Count total için pagination
        total =query .count ()

        # ERSIN Apply pagination
        from sqlalchemy .sql import ColumnElement 
        from typing import cast 
        created_at_col =cast (ColumnElement [Any ],Analysis .created_at )
        items =query .order_by (desc (created_at_col )).offset ((page -1 )*per_page ).limit (per_page ).all ()
...
def bulk_update_training_status (feedback_ids :List [int ],status :str ,model_version :str |None =None ):
    """
    ...
    try :
        ...
        # ERSIN SQLAlchemy update() accepts string keys in SQLAlchemy 2.0, but type checker doesn't recognize it
        ...
        if model_version :
            ...
            # ERSIN Use helper function for .in_() method
        filter_expr =column_in (Feedback .id ,feedback_ids )
        # ERSIN SQLAlchemy's .update() method exists and works correctly at runtime
        feedback_query =db .session .query (Feedback ).filter (filter_expr )
        # ERSIN Use update() method - SQLAlchemy Query.update() accepts dict[str, Any] at runtime
        from sqlalchemy .orm .query import Query 
        from typing import cast 
        if isinstance (feedback_query ,Query ):
        # ERSIN Cast to Any to avoid type checker issues with dict[str, Any] vs Dict[_DMLColumnArgument, Any]
            cast (Query [Any ],feedback_query ).update (cast (Any ,update_data ),synchronize_session =False )
        else :
        # ERSIN Fallback: update records individually (should not happen in practice)
        # ERSIN Use getattr to safely access .all() method
            all_method =getattr (feedback_query ,'all',None )
            if all_method is not None and callable (all_method ):
                feedbacks_list =all_method ()
                # ERSIN Type checker doesn't know feedbacks_list is iterable, cast to list[Any]
                feedbacks =cast (list [Any ],feedbacks_list )
