...
# ERSIN Queue backend
# ERSIN - "memory": mevcut in-process queue (tek proses için uygun)
# ERSIN - "redis": web ve worker proseslerini ayırmak için (önerilen prod)
...
def _get_redis ():
    """Get Redis client (sync, not async)"""
    ...
    try :
        ...
        # ERSIN Redis client is sync, not async - type checker may see it as awaitable incorrectly
        _redis_client =redis .Redis .from_url (REDIS_URL ,decode_responses =True )
        return cast (Any ,_redis_client )# ERSIN Cast to Any to avoid awaitable type issues
...
def _set_worker_state (is_processing_value :bool ,active_analyses :int ):
    ...
    try :
        ...
        # ERSIN Global analiz kuyruğu (memory backend)
analysis_queue =queue .Queue ()
# ERSIN İşleme kilidi (memory backend)
...
def _acquire_gpu_lock ():
    ...
    # ERSIN Non-blocking acquire loop (eventlet uyumlu)
    while True :
        try :
            ...
            if e .errno not in (errno .EAGAIN ,errno .EACCES ):
                ...
                # ERSIN Lock busy -> yield/sleep
            try :
            # ERSIN Eventlet is an optional dependency, import safely
                import eventlet 

                eventlet .sleep (int (0.5 ))
            except Exception :
                time .sleep (int (0.5 ))
...
def database_session (app ):
    ...
    try :
        with app .app_context ():
            ...
            # ERSIN Yeni session başlat
            session =db .session 

            # ERSIN İşlem başlangıcında session'ı temizle
            session .rollback ()# ERSIN Önceki işlemlerden kalan uncommitted changes'i temizle

            # ERSIN Kritik: SQLite WAL mode'da diğer process'lerin commit'lerini görmek için
            # ERSIN session'ı expire ederek fresh data çekmesini sağla
            session .expire_all ()

            # ERSIN Yield session (close yapmadan önce)
            yield session 

            # ERSIN Başarılı işlem sonrası commit
            if session is not None :
                session .commit ()

    except Exception as e :
    # ERSIN Hata durumunda rollback
        if session is not None :
            try :
                session .rollback ()
    ...
    # ERSIN Her durumda session'ı temizle (yield'den sonra)
        if session is not None :
            try :
                session .close ()
            except :
...
def add_to_queue (analysis_id ):
    ...
    if is_redis_backend ():
    # ERSIN Cross-process queue: web sadece enqueue eder; worker BLPOP ile tüketir.
        r =_get_redis ()
        r .rpush (REDIS_QUEUE_KEY ,str (analysis_id ))
        emit_queue_status ()
        return 

        # ERSIN Fallback: in-process queue (dev)
...
def emit_queue_status ():
    ...
    try :
        ...
        # ERSIN WebSocket ile kuyruk durumu bildirimi gönder
        try :
        ...
        # ERSIN HTTP endpoint /api/queue/status hala mevcut
    except Exception as e :
...
def start_processor ():
    ...
    if is_redis_backend ():
    # ERSIN Redis backend'te queue processing ayrı worker process'te yapılır.
        logger .info ("Redis queue backend aktif: in-process queue processor başlatılmıyor.")
        return 
    with processing_lock :
        if not is_processing :
            is_processing =True 
            # ERSIN Gunicorn eventlet worker altında OS thread yerine eventlet greenlet kullanmak daha güvenli.
            # ERSIN ERSIN Özellikle SocketIO emit'leri background task içinde çalıştığı için, eventl...
            try :
            # ERSIN Eventlet is an optional dependency, import safely
                import eventlet 
...
def process_queue ():
    ...
    try :
    # ERSIN Ana Flask app'i globalden al ve context aç
        from app import global_flask_app 
        # ERSIN db ve get_socketio şu an kullanılmıyor
        logger .info ("Kuyruk işleyici çalışıyor. Global Flask app context açılıyor.")
        with global_flask_app .app_context ():
            while not analysis_queue .empty ():
            # ERSIN Kuyruk durumu bildirimi gönder
                emit_queue_status ()
            ...
            # ERSIN Son kuyruk durumu bildirimi
            emit_queue_status ()
    ...
    # ERSIN İşleme durumunu sıfırla
        with processing_lock :
            is_processing =False 
            logger .info ("Kuyruk işleyici durduruldu.")

            # ERSIN Eğer kuyrukta hala eleman varsa, yeni bir işleyici başlat
...
def process_one_analysis (analysis_id :str ,app =None )->Tuple [bool ,str ]:
    ...
    """
    # ERSIN Worker state (redis) - best effort
    ...
    try :
        ...
        # ERSIN Analizin varlığını/iptalini kontrol et
            from app .models .analysis import Analysis 
            analysis_file_id =None 

            with database_session (target_app ):  # pyright: ignore[reportUnknownMemberType]
                from app import db 
                analysis =Analysis .query .get (analysis_id )
                if not analysis :
                    # ERSIN Kısa gecikme + tekrar dene (commit/visibility race)
                    time .sleep (0.5 )
                    db .session .expire_all ()
                    analysis =Analysis .query .get (analysis_id )
                if not analysis :
                    db_path =target_app .config .get ("SQLALCHEMY_DATABASE_URI","")
                    logger .error (f"Analiz bulunamadı: {analysis_id } (DB: {db_path })")
                ...
                if getattr (analysis ,"is_cancelled",False ):
                    ...
                    # ERSIN Mark as processing + set start_time için observability.
                try :
            ...
            try :
                ...
                # ERSIN Açıklama: Not:
                # ERSIN subprocess.run(...) blocks için  whole duration of  analysis.
                # ERSIN Our worker heartbeat keys in Redis have TTL (ex=60). için longer analyses (videos),
                # ERSIN keys expire ve /api/queue/stats shows worker_last_heartbeat=null although
                # ERSIN Worker alive ve processing, heartbeat'i yenilemek için Popen + poll loop kullan
                ...
                # ERSIN Subprocess proje kökünde çalışsın (module import ve DB yolları tutarlı)
                project_root =os .path .abspath (os .path .join (os .path .dirname (__file__ ),os .pardir ,os .pardir ))
                subprocess_cwd =project_root 
                worker_db_uri =target_app .config .get ("SQLALCHEMY_DATABASE_URI","")
                logger .info (
                    f"Subprocess başlatılıyor: analysis_id={analysis_id }, cwd={subprocess_cwd}, worker_db={worker_db_uri}"
                )
                # ERSIN Subprocess'in aynı DB dosyasını kullanması: env + argv (argv kesin taşınır)
                subprocess_env =dict (os .environ )
                # ERSIN Python module resolution için proje kökünü ekle
                existing_pythonpath =(subprocess_env .get ("PYTHONPATH")or "").strip ()
                if project_root not in existing_pythonpath .split (":")if existing_pythonpath else []:
                    subprocess_env ["PYTHONPATH"]=(project_root +(":"+existing_pythonpath if existing_pythonpath else ""))
                if worker_db_uri :
                    subprocess_env ["DATABASE_URL"]=worker_db_uri
                cmd =[sys .executable ,"-m","app.services.analysis_subprocess_runner",str (analysis_id )]
                if worker_db_uri :
                    cmd .append (worker_db_uri)
                ...
                while True :
                    ...
                    if rc is not None :
                        ...
                        # ERSIN Refresh heartbeat while processing (best effort)
                    _set_worker_state (True ,1 )

                    # ERSIN Hard timeout (1 hour)
                    ...
                    # ERSIN Process'in tamamen sonlandırıldığından ve stream'lerin boşaltıldığından emin ol
                try :
                    proc .wait (timeout =5 )
                except Exception :
                    pass 

                    # ERSIN Bazı native deps (örn. insightface/onnxruntime) stdout'a print edebilir
                    # ERSIN Bu "last-line-is-JSON" varsayımını bozabilir, stdout'ta bulabildiğimiz son geçerli JSON object'i parse et
                    # ERSIN Gerekirse stderr'e fallback yap, çıktıları zaten stdout_lines/stderr_lines'a stream ettik
                ...
                # ERSIN Give a helpful debug snippet (last non-empty line).
                    last_line =""
            ...
            if not success and stderr_lines :
                for line in stderr_lines :
                    if (line or "").strip ():
                        logger .error (f"Subprocess stderr: {line }")

            # ERSIN Final durumu güncelle
            with database_session (target_app ):  # pyright: ignore[reportUnknownMemberType]
                # ERSIN session context manager için gerekli ama şu an kullanılmıyor
                analysis =Analysis .query .get (analysis_id )
                if analysis :
                    from datetime import datetime 
                    ...
                    if success :
                        ...
                        # ERSIN keep message için UI/debugging
                        try :
...
def _emit_analysis_status (analysis_id ,file_id ,status ,progress ,message ):
    ...
    try :
    # ERSIN Bu fonksiyon artık kullanılmıyor - yeni WebSocket sistem aktif
...
def remove_cancelled_from_queue (app =None ):
    ...
    """
    try :
    # ERSIN Prefer existing Flask app context if available; otherwise use provided app veya global fallback.
        ...
        if is_redis_backend ():
        # ERSIN Redis list üzerinde basit bir filtreleme (küçük kuyruklarda yeterli)
            from app .models .analysis import Analysis 
            r =_get_redis ()
            removed_count =0 

            if has_app_context ():
            # ERSIN Redis client is sync, lrange returns list directly (not awaitable)
                items_result =r .lrange (REDIS_QUEUE_KEY ,0 ,-1 )
                queue_items :list [str ]=items_result if isinstance (items_result ,list )else []
                kept =[]
                for analysis_id in queue_items :
                    analysis =Analysis .query .get (analysis_id )
                ...
                with target_app .app_context ():
                # ERSIN Redis client is sync, lrange returns list directly (not awaitable)
                    items_result_2 =r .lrange (REDIS_QUEUE_KEY ,0 ,-1 )
                    queue_items_2 :list [str ]=items_result_2 if isinstance (items_result_2 ,list )else []
                    kept =[]
                    for analysis_id in queue_items_2 :
                        analysis =Analysis .query .get (analysis_id )
                        if analysis and analysis .is_cancelled :
                            removed_count +=1 
                        else :
        ...
        # ERSIN Kuyruktaki tüm analizleri kontrol et
        if has_app_context ():
            while not analysis_queue .empty ():
                try :
                    analysis_id =analysis_queue .get_nowait ()

                    # ERSIN Analizin iptal edilip edilmediğini kontrol et
                    analysis =Analysis .query .get (analysis_id )
                    if analysis and analysis .is_cancelled :
                        logger .info (f"🗑️ Kuyruktan iptal edilmiş analiz temizlendi: #{analysis_id }")
                        removed_count +=1 
                    else :
                    # ERSIN İptal edilmemişse geri kuyruğa koy
                    ...
                    # ERSIN Temizlenmiş kuyruğu geri yükle
...
def get_queue_status ():
    ...
    if is_redis_backend ():
        try :
            r =_get_redis ()
            # ERSIN Redis client is sync, llen and get return values directly (not awaitable)
            llen_result =r .llen (REDIS_QUEUE_KEY )
            qsize =int (llen_result if isinstance (llen_result ,int )else (llen_result or 0 ))
            get_result =r .get (REDIS_WORKER_PROCESSING_KEY )
            is_proc =(get_result if isinstance (get_result ,str )else (get_result or "0"))=="1"
            return {
            'queue_size':qsize ,
...
def cleanup_queue_service ():
    ...
    try :
        ...
        if is_redis_backend ():
        # ERSIN Redis backend'te cleanup worker prosesin sorumluluğunda.
            logger .info ("Redis queue backend aktif: in-process cleanup atlandı.")
            return 

            # ERSIN İşleme durumunu durdur
        with processing_lock :
            is_processing =False 

            # ERSIN Kuyruktaki bekleyen işleri temizle
        while not analysis_queue .empty ():
            try :
                analysis_id =analysis_queue .get_nowait ()
...
def clear_queue ():
    ...
    if is_redis_backend ():
        try :
            r =_get_redis ()
            # ERSIN Del -> önce uzunluğu al
            # ERSIN Redis client is sync, llen returns int directly (not awaitable)
            llen_result =r .llen (REDIS_QUEUE_KEY )
            cleared_count =int (llen_result if isinstance (llen_result ,int )else (llen_result or 0 ))
            ...
            # ERSIN Önce işleme durduralım
    with processing_lock :
        is_processing =False 

        # ERSIN Kuyrukta bekleyen tüm analizleri temizle
        try :
            while True :
