...
class EnsembleIntegrationService :
    ...
    def predict_age_enhanced (self ,base_age_prediction :float ,face_embedding :np .ndarray [Any ,Any ]|None =None )->dict [str ,Any ]:
        """
    ...
    def predict_content_enhanced (self ,base_description :str ,base_confidence :float ,content_id :str |None =None ,person_id :str |None =None ,clip_embedding :list [Any ]|None =None )->dict [str ,Any ]:
        """
        ...
        try :
        # ERSIN Convert types: content_id and person_id from str to int, clip_embedding from list to np.ndarray
            import numpy as np 
            content_id_int :int |None =None 
            person_id_int :int |None =None 
            clip_embedding_array :np .ndarray [Any ,Any ]|None =None 
            ...
            if clip_embedding is not None :
                try :
                    clip_embedding_array =np .array (clip_embedding )
                except (ValueError ,TypeError ):
                    clip_embedding_array =None 

            final_desc ,final_conf ,info =self .clip_ensemble .predict_content_ensemble (
            base_description ,base_confidence ,content_id_int ,person_id_int ,clip_embedding_array 
            )
    ...
    def analyze_image_enhanced (self ,image :str ,face_data :dict [str ,Any ],content_data :dict [str ,Any ])->dict [str ,Any ]:
        """
        ...
        'enhanced_faces':[],# ERSIN list[dict[str, Any]]
        'enhanced_content':{},# ERSIN dict[str, Any]
        'ensemble_stats':{# ERSIN dict[str, int]
        ...
        # ERSIN Enhance face predictions
        if 'faces'in face_data :
            for face in face_data ['faces']:
                base_age =face .get ('age',25 )
                embedding =face .get ('embedding')

                # ERSIN Convert embedding to numpy array if it's a list
                ...
                # ERSIN Type checker için enhanced_faces list'ini güvenli şekilde kullan
                enhanced_faces_list =results .get ('enhanced_faces',[])
                if isinstance (enhanced_faces_list ,list ):
                    enhanced_faces_list .append (enhanced_face )
                    results ['enhanced_faces']=enhanced_faces_list

                if enhanced_age ['method']!='base_model_only':
                    results ['ensemble_stats']['age_corrections_applied']+=1 

                    # ERSIN Enhance content predictions
        if 'content'in content_data :
            ...
            if enhanced_content ['method']!='base_model_only':
                ...
                # ERSIN Calculate total improvements
    ...
    def get_system_status (self )->dict [str ,Any ]:
        ...
        'capabilities':{
        'age_correction':int (age_stats .get ('total_people_corrections',0 ))>0 ,
        'content_correction':int (clip_stats .get ('total_content_corrections',0 ))>0 ,
        'confidence_adjustment':int (clip_stats .get ('total_confidence_adjustments',0 ))>0 ,
        'embedding_similarity':(
        int (age_stats .get ('total_embedding_corrections',0 ))>0 or 
        int (clip_stats .get ('total_embedding_corrections',0 ))>0 
        )
        }
        }

    def refresh_corrections (self )->dict [str ,Any ]:
        """Refresh all ensemble corrections and create model versions"""
        try :
        # ERSIN İlk önce feedback'lerden düzeltmeleri yükle
            ...
            # ERSIN Ensemble düzeltmelerini model versiyonu olarak kaydet
            age_version =None 
            clip_version =None 

            try :
                if age_result >0 :# ERSIN Eğer yaş düzeltmeleri varsa
                    ...
                    # ERSIN Correction sonrası yaş modeli eğitimi doğrudan zincir içinde başlat
                    try :
            ...
            try :
            # ERSIN clip_result is int (number of corrections loaded)
                clip_result_int =int (clip_result )if isinstance (clip_result ,(int ,float ,str ))else 0 
                if clip_result_int >0 :# ERSIN Eğer içerik düzeltmeleri varsa
                    ...
                    # ERSIN Eğitimde kullanılan ve correction zincirine girmeyen feedback'leri de temizle
                    try :
    ...
    def reset_ensemble_corrections (self ,model_type :str )->dict [str ,Any ]:
        """Reset ensemble corrections for specified model type"""
        try :
            if model_type =='age':
            # ERSIN Clear age corrections
    ...
    def get_real_age_distribution (self )->dict [str ,Any ]:
        """Get real-time age distribution from feedback database"""
        try :
            from app import db 
            from app .models import Feedback 

            # ERSIN Query real feedback data - only age feedbacks
            ...
            # ERSIN Fallback to empty distribution
            return {"0-10":0 ,"11-20":0 ,"21-30":0 ,"31-40":0 ,"41-50":0 ,"51-60":0 ,"61+":0 }

    def get_real_error_distribution (self )->dict [str ,Any ]:
        ...
        try :
            ...
            for feedback in feedbacks :
                if ensemble_active :
                # ERSIN Ensemble aktifse çoğunlukla perfect
                    person_id =feedback .person_id 
                    if person_id and person_id in self .age_ensemble .feedback_corrections :
                        error_ranges ["0 yaş fark"]+=1 # ERSIN Perfect correction
                    else :
                    # ERSIN Base model error
                        original_age =feedback .pseudo_label_original_age or 25 # ERSIN fallback
            ...
            # ERSIN Fallback to default
            return {"0 yaş fark":1 ,"1 yaş fark":1 ,"2 yaş fark":1 ,"3+ yaş fark":1 }

            # ERSIN Global ensemble service instance
