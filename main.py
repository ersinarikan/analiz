#!/usr/bin/env python3
"""
WSANALIZ Flask Application Entry Point
"""
import eventlet 
# ERSIN Not: PyTorch/OpenCLIP gibi native kütüphaneler eventlet'in thread patch'i ile bazı ortamlarda segfault'a yol açabiliyor
# ERSIN Bu nedenle thread patch kapalı tutulur
eventlet .monkey_patch (thread =False )
import sys 
import os 
import logging 
import signal 
import threading 

# ERSIN Thread-safe logging lock
_log_lock =threading .Lock ()

logger =logging .getLogger ("wsanaliz.app")
# ERSIN Logging seviyesi environment'a göre ayarlanacak

# ERSIN TensorFlow uyarılarını bastır
os .environ ['TF_CPP_MIN_LOG_LEVEL']='2'# ERSIN INFO ve WARNING loglarını gizle

try :
    import tensorflow as tf 
    tf .get_logger ().setLevel ('ERROR')# ERSIN Sadece ERROR loglarını göster
except ImportError :
    tf =None 
    logger .warning ("TensorFlow bulunamadı, devam ediliyor...")

    # ERSIN Flask uygulamasını import et
try :
    from app import create_app ,initialize_app 
except ImportError as import_err :
    create_app =None 
    initialize_app =None 
    logger .error (f"Flask uygulaması import edilemedi: {import_err }")
    logger .error ("Virtual environment'ı aktifleştirip tekrar deneyin:")
    logger .error ("   venv\\Scripts\\activate  (Windows)")
    logger .error ("   source venv/bin/activate  (Linux/Mac)")
    sys .exit (1 )

PID_FILE ="wsanaliz.pid"

def write_pid ():
    with open (PID_FILE ,"w")as f :
        f .write (str (os .getpid ()))

def remove_pid ():
    try :
        os .remove (PID_FILE )
    except Exception :
        pass 

def signal_handler (_signum ,_frame ):
    """Graceful shutdown handler"""
    logger .info ("Shutdown signal alındı...")
    try :
    # ERSIN Background services'ları kapat
        logger .info ("Background servisler kapatılıyor...")
        # ERSIN Queue service'yi kapat
        try :
            from app .services .queue_service import cleanup_queue_service 
            cleanup_queue_service ()
        except Exception as queue_err :
            logger .warning (f"Queue service kapatma hatası: {queue_err }")
            # ERSIN Memory cleanup
        logger .info ("Memory cleanup yapılıyor...")
        import gc 
        gc .collect ()
        logger .info ("Graceful shutdown tamamlandı!")
    except Exception as shutdown_err :
        logger .error (f"Shutdown sırasında hata: {shutdown_err }",exc_info =True )
    finally :
        sys .exit (0 )# ERSIN Force exit

if __name__ =="__main__":
    if len (sys .argv )>1 :
        if sys .argv [1 ]=="-stop":
            from app .utils .restart_helper import stop_app 
            stop_app ()
        elif sys .argv [1 ]=="-start":
            from app .utils .restart_helper import start_app 
            start_app ()
        else :
            print ("Kullanım: python main.py [-stop|-start]")
    else :
    # ERSIN Signal handlers ekle
        signal .signal (signal .SIGINT ,signal_handler )# ERSIN Ctrl+C
        signal .signal (signal .SIGTERM ,signal_handler )# ERSIN Terminal

        try :
            write_pid ()
            logger .info ("WSANALIZ Flask Uygulaması Başlatılıyor...")

            app ,socketio_direct =create_app (return_socketio =True )# ERSIN (app, socketio)
            initialize_app (app )# ERSIN Sadece ana süreçte çalıştırılacak

            # ERSIN Werkzeug HTTP request loglarını aç, debug için
            log =logging .getLogger ('werkzeug')
            log .setLevel (logging .INFO )# ERSIN HTTP requestleri göster

            # ERSIN Environment'a göre debug mode belirle
            environment =os .environ .get ('FLASK_ENV','development')
            is_debug =environment =='development'

            # ERSIN Logging seviyesini ayarla
            if is_debug :
                logging .basicConfig (level =logging .DEBUG )
            else :
                logging .basicConfig (level =logging .INFO )

            if is_debug :
                logger .info ("Development mode: Debug ve auto-reload aktif")
            else :
                logger .info ("Production mode: Debug kapalı, performans optimized")

            logger .info ("Uygulama başarıyla başlatıldı!")
            logger .info ("Erişim: http://localhost:5000")
            logger .info ("Durdurmak için: Ctrl+C")

            logger .info ("SocketIO server başlatılıyor...")
            # ERSIN SocketIO ile çalıştır
            socketio_direct .run (app ,
            host ='0.0.0.0',
            port =5000 ,
            debug =is_debug ,# ERSIN Environment'a göre debug mode
            allow_unsafe_werkzeug =True )

        except KeyboardInterrupt :
            logger .info ("Keyboard interrupt alındı...")
            signal_handler (None ,None )
        except Exception as main_err :
            logger .error (f"Uygulama başlatılırken hata: {main_err }",exc_info =True )
            logger .error ("Çözüm önerileri:")
            logger .error ("   1. Virtual environment'ı aktifleştirin")
            logger .error ("   2. Gerekli paketleri yükleyin: pip install -r requirements.txt")
            logger .error ("   3. Veya flask run --debug komutunu kullanın")
            sys .exit (1 )
