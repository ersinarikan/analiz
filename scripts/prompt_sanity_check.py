...
def main(argv: list[str]) -> int:
    ...
    # Keep stdout clean for JSONL output: route logs/prints to stderr.
    # Some parts of app initialization use print() (not logging), so we redirect stdout during init.
    os.environ.setdefault("LOG_LEVEL", "WARNING")
    # Sanity-check should be lightweight and not fight with the running production services.
    # Default to CPU + disable memory management unless user explicitly overrides via env.
    ...
    # Import + init app/analyzer with stdout redirected to stderr to avoid contaminating JSON output.
    with contextlib.redirect_stdout(sys.stderr):
        from app import create_app
        from app.ai.content_analyzer import ContentAnalyzer

        created = create_app("default")
        # create_app may return (app, socketio) in some contexts; support both
    ...
    try:
        ...
        with flask_app.app_context():
            ...
            for p in paths:
                try:
                    # returns: (violence, adult_content, harassment, weapon, drug, safe, ...) + objects
                    result = analyzer.analyze_image(str(p))
                    # analyzer returns a long tuple; normalize into dict
                    # See content_analyzer.py: all_category_keys_for_return = keys + ['safe']
                    categories = list(analyzer.category_prompts.keys()) + ["safe"]
                    scores = {k: float(v) for k, v in zip(categories, result[: len(categories)])}
