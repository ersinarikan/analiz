...
def main(argv: list[str]) -> int:
    ...
    with open(args.out, "a", encoding="utf-8") as out_f:
        while True:
            t0 = _now()
            # We prefer in-flight visibility, so we enumerate files and query /api/analysis/file/<file_id>.
            ...
            for a in analyses:
                ...
                if status in FINAL_STATUSES and not entry.get("final_logged"):
                    # Fetch full analysis record for timestamps/message
                    detail: dict[str, Any] = {}
                    try:
