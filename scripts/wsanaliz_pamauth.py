...
# Sadece bu UID'den gelen bağlantılar kabul (wsanaliz-web User=ersin → 1001)
try:
    ALLOW_UID = int(os.environ.get("WSANALIZ_PAMAUTH_ALLOW_UID", "1001").strip())
except Exception:
    ALLOW_UID = 1001

# Bağlantıda soket grubunu ayarlamak için (ersin'in gid'i)
...
def struct_size() -> int:
    ...
    # Linux: 4+4+4 = 12 on 32-bit; on 64-bit often 4+4+4 (uint32 each) = 12
    return 12
...
def parse_peercred(data: bytes) -> tuple[int, int, int]:
    ...
    # ucred: I=uint32 for pid, uid, gid (Linux)
