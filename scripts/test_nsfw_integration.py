...
def test_nsfw_integration():
    ...
    try:
        ...
        with app.app_context():
            ...
            test_image[100:200, 100:200] = [255, 0, 0]  # Kırmızı kare
            logger.info("✅ Test görüntüsü oluşturuldu (480x640)")
            
            # 4. analyze_image testi (NSFW enabled)
            ...
            try:
                ...
                # adult_content skorunun NSFW'den geldiğini kontrol et
                if app.config.get('NSFW_ENABLED', False):
                    ...
                    if nsfw_model is not None:
                        ...
                        # Skorlar yakın olmalı (NSFW'den geliyorsa)
            ...
            # 6. Kategori kontrolü (adult_content CLIP döngüsünden çıkarılmış mı?)
            logger.info("\n[TEST 6] Kategori kontrolü...")
            categories = list(analyzer.category_prompts.keys())
            logger.info(f"   - Tüm kategoriler: {categories}")
            
            if app.config.get('NSFW_ENABLED', False):
                # NSFW enabled ise, analyze_image içinde adult_content çıkarılmalı
                # Bu test için analyze_image içindeki categories listesini kontrol edemeyiz
                # Ama loglardan anlaşılabilir
